...
def test_summarize_runs_rejects_first_non_object_run():
    with pytest.raises(ValueError, match="measured run must be an object"):
        PERF.summarize_runs(["run"])


def test_summarize_runs_rejects_later_non_object_run_before_profile_equality():
...
@pytest.mark.parametrize("missing_index", [0, 1])
def test_summarize_runs_rejects_first_or_later_missing_long_context_profile(missing_index):
    payload = native_payload(activation="f32", selected=(358, 1184))
    run = PERF.compact_run(
        {"wall_elapsed_seconds": 3.5, "peak_rss_bytes": 1024, "payload": payload},
        activation="f32",
    ...
    runs = [dict(run), dict(run)]
    runs[missing_index].pop("long_context_profile")
...
def test_build_long_context_measurement_gate_rejects_zero_rss_count():
    profile = {
        "enabled": True,
        "policy": "ctx4k-smoke",
        "target_ctx_tokens": 4096,
    ...
    cells = [{"summary": {"long_context_profile": profile, "peak_rss_bytes": {"count": 0}}}]

    with pytest.raises(ValueError, match="peak_rss_bytes count must be positive"):
        PERF.build_long_context_measurement_gate(cells, ctx=4096)


def test_build_long_context_measurement_gate_rejects_missing_rss_count():
    profile = {
        "enabled": True,
        "policy": "ctx4k-smoke",
        "target_ctx_tokens": 4096,
    ...
    cells = [{"summary": {"long_context_profile": profile, "peak_rss_bytes": {}}}]

    with pytest.raises(ValueError, match="peak_rss_bytes count is required"):
        PERF.build_long_context_measurement_gate(cells, ctx=4096)
