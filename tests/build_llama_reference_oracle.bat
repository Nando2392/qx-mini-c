...
if errorlevel 1 exit /b %errorlevel%
cl /nologo /std:c++17 /O2 /W4 /EHsc /D_CRT_SECURE_NO_WARNINGS /DLLAMA_COMMIT=\"%LLAMA_COMMIT%\" /I"%LLAMA%\include" /I"%LLAMA%\src" /I"%LLAMA%\ggml\include" tests\llama_sequence_oracle.cpp /Fe:build\llama_sequence_oracle.exe /link /LIBPATH:"%LLAMA%\build-k3-cpu\src\Release" /LIBPATH:"%LLAMA%\build-k3-cpu\ggml\src\Release" llama.lib ggml-cpu.lib ggml-base.lib ggml.lib
if errorlevel 1 exit /b %errorlevel%
