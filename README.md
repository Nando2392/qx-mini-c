...
Current fixed-token result against llama.cpp commit `768d2a481a99cb75ec9a03b95dadbd35e7acf496`: exact end-to-end numerical parity remains **refuted**. The backward same-input evidence closes layer 47 and layers 44–42, then found the first material local failure in that descending run at layer 41: `ffn_moe_down` used F32 fallback for an IQ3_S tensor while ggml uses a Q8_K activation. The new CPU opt-in `IQ3_S × Q8_K` kernel closes reconstructed `l_out-41` at max-abs `4.76e-5`, RMSE `1.05e-6`, cosine ≈`1`, with identical top-8. The opt-in final head uses `Q6_K × Q8_K` and complete global logits remain max-abs `0.132130`, RMSE `0.0257469`, cosine `0.999967105`, with argmax `1124`. F32 remains the default dequantized path. See [`wiki/comparisons/layer41-iq3s-q8k.md`](wiki/comparisons/layer41-iq3s-q8k.md), [`wiki/comparisons/layer47-same-input.md`](wiki/comparisons/layer47-same-input.md) and [`wiki/comparisons/final-head-q6k-q8k.md`](wiki/comparisons/final-head-q6k-q8k.md).
...
1. Continue the same-input backward bisect before layer 41; layer 47 and layers 44–42 close, and the layer-41 IQ3_S down mismatch is fixed independently.
