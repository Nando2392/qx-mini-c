...
chat_fail:
    ...
    if (strcmp(argv[1], "prompt-state-loop-probe") == 0) {
        ...
        uint64_t long_context_soak_seconds = 0u;
        ...
        for (int i = 2; i < argc; ++i) {
            ...
            else if (strcmp(argv[i], "--kv2-policy") == 0 && i + 1 < argc) kv2_policy = argv[++i];
            else if (strcmp(argv[i], "--sampling-policy") == 0 && i + 1 < argc) sampling_policy = argv[++i];
            ...
            else if (strcmp(argv[i], "--long-context-soak-seconds") == 0 && i + 1 < argc) {
                if (!qx_cli_parse_u64_arg("--long-context-soak-seconds", argv[++i], &long_context_soak_seconds, err, sizeof(err))) {
                    fprintf(stderr, "prompt-state-loop-probe failed: %s\n", err); return 2;
                }
            }
        ...
        if (strcmp(speculative_policy, "none") != 0) {
            fprintf(stderr, "prompt-state-loop-probe failed: unsupported speculative policy\n"); return 2;
        }
        if (strcmp(kv2_policy, "none") != 0) {
            fprintf(stderr, "prompt-state-loop-probe failed: unsupported KV2 policy\n"); return 2;
        }
        ...
        if (long_context_soak_seconds != 0u) {
            fprintf(stderr, "prompt-state-loop-probe failed: long-context soak seconds are not implemented\n"); return 2;
        }
