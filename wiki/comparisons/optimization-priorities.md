...
Issue #78 fija ese snapshot F32 y reanuda token 1000 en layer 2. Los controles F32/F32 y Q8_K/Q8_K son exactos. Con el residual F32 exacto de salida de layer 1, el sufijo Q8_K aún selecciona `1318`; el routing de layer 2 coincide con F32 y la primera salida de routing distinta reaparece en layer 3. Ninguna comparación full-logit del diagnóstico pasa. El siguiente gate debe inyectar la salida exacta de layer 2 al iniciar layer 3; no se atribuye root cause a un kernel ni se promueve `q8_k_compat`.
...
Issue #81 convierte el loop ya validado en una superficie de generación CPU canónica sin añadir un backend nuevo: CLI desde texto con tokenizer QXT ligado y salida JSON, más API C compartida. El gate real reproduce `Hello!` → `[358,1184]`; el API controla EOS en primer/segundo token o lo deshabilita con `-1`. La API compatible conserva 64 posiciones; la API caller-buffer posterior admite hasta 4096 y `<=ctx`. Esto prioriza una interfaz reproducible de inferencia antes de optimizar: no prueba throughput, paridad global, CUDA ni una corrida/quality/soak 4K. Issue #81 está CLOSED en `0305290b3aba00d9db62f32caacfbc2e14cdbeb`; CI `35412907586` pasó. Evidencia: `wiki/evidence/issue-81-native-generation-report.json`.
